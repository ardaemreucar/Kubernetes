# Kubernetes Hands-On Day-1 :Installation


## Part 1 - Setting Up Kubernetes Environment

- In this hands-on, we will prepare two nodes for Kubernetes on `Ubuntu 20.04`. Choose instances of type t3a.small. One of the node will be configured as the Master node, the other will be the worker node. Following steps should be executed on all nodes. 
...
```bash
sudo kubeadm init --apiserver-advertise-address=172.31.76.33 --pod-network-cidr=10.244.0.0/16
```
